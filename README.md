Вёрстка сайта - шаблона строительной компании createx.

Сайт доступен для desktop (1440p), mobile версия в разработке.

Чтобы запустить проект, его необходимо склонировать, перейти в корневую папку и прописать пару команд: npm i, gulp.

Так как это многостраничный сайт, чтобы переходить на разные страницы, в строке браузера можно добавлять названия html документов из папки app проекта (например /work.html).
